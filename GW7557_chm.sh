...
ARM_BUILD_BUILD_VER="3" #This just only support number
ARM_VENDOR_BUILD_VER=""
ARM_IS_THE_FIRMWARE_GA_RELEASE="y"
else
ARM_PATCH_BUILD_VER="0"
ARM_BUILD_BUILD_VER="14"
ARM_VENDOR_BUILD_VER=""
ARM_IS_THE_FIRMWARE_GA_RELEASE="n"
fi
ARM_SELECT_BUILD_PROFILE="gw7557c_lgi" #with lower case
if [ -n "${ARM_VENDOR_BUILD_VER}" ]; then
VERSION="6.12.${ARM_PATCH_BUILD_VER}.${ARM_BUILD_BUILD_VER}${ARM_VENDOR_BUILD_VER}"
else
VERSION="6.12.${ARM_PATCH_BUILD_VER}.${ARM_BUILD_BUILD_VER}"
CUSTOMER_NAME="GW7557CE"
fi
echo -e "\n"
...
ATOM_SELECT_BUILD_PROFILE=${CUSTOMER_NAME} #Upper case is ok
ATOM_IS_THIS_CORRECT="y"

####################parameters for source code control####################
HOME_FOLDER="/home/${SSH_ACCOUNT}"
BUILD_FOLDER=`pwd`
VERSION_PROJECT="${CUSTOMER_NAME}-${VERSION}"
...
ARM_BRANCH_NAME="develop"
ATOM_BRANCH_NAME="MU-MIMO_CL2400_4.7x"
...
ATOM_IMAGE_PATH="products/configs/images/${CUSTOMER_NAME}-4.50.18.${ATOM_BUILD_BUILD_VER}${ATOM_VENDOR_BUILD_VER}"
ARM_ORI_SH_IMAGE_FILENAME="${VERSION_PROJECT}-SH"
ATOM_ORI_SH_IMAGE_FILENAME="${CUSTOMER_NAME}-4.50.18.${ATOM_BUILD_BUILD_VER}${ATOM_VENDOR_BUILD_VER}-NOSAS-SH_appcpuImage"
...
ATOM_TEMP_IMAGE_FILENAME="${ARM_SELECT_BUILD_PROFILE}_${VERSION}-${BUILD_DATE}_ubfi.img" #gw7557c_lgi_6.12.0.2-2-20200527_ubfi.img_

ARM_ORI_NOSH_IMAGE_FILENAME="${VERSION_PROJECT}-NOSH"
ATOM_ORI_NOSH_IMAGE_FILENAME="${CUSTOMER_NAME}-4.50.18.${ATOM_BUILD_BUILD_VER}${ATOM_VENDOR_BUILD_VER}-NOSAS-NOSH_appcpuImage"
ARM_NOSH_IMAGE_FILENAME="${VERSION_PROJECT}-NOSH"
ATOM_NOSH_IMAGE_FILENAME="${ARM_NOSH_IMAGE_FILENAME}_appcpuImage"


ENCRYPTIMAGE_TOOL_FOLDERNAME="cbnEncryptImage"
#KEY_FOLDER="${HOME_FOLDER}/${CUSTOMER_NAME}_RSA_Keys"
...
function Sync_SH_Image_To_Release_Folder(){
    ...
    #copy ATOM file to Release folder
    cp "${ENCRYPTIMAGE_TOOL_FOLDERNAME}/${VERSION_PROJECT}-SH" ${RELEASE_FOLDER}
...
function Sync_NOSH_Image_To_Release_Folder(){
    ...
    #copy ATOM file to Release folder
    cp "${ENCRYPTIMAGE_TOOL_FOLDERNAME}/${VERSION_PROJECT}-NOSH" ${RELEASE_FOLDER}
    #copy ARM file to Release folder
...
function Generate_FW_Image_SH(){
    ...
    cp "${ARM_OUTPUT_IMAGE_PATH}/${VERSION_PROJECT}-SH" ${ENCRYPTIMAGE_TOOL_FOLDERNAME}

    cd ${ENCRYPTIMAGE_TOOL_FOLDERNAME}
    ./cbnEncryptImage -i "${VERSION_PROJECT}-SH-PA" -o "${VERSION_PROJECT}-SH.p7"
...
function Generate_FW_Image_NOSH(){
    ...
    cp "${ARM_OUTPUT_IMAGE_PATH}/${VERSION_PROJECT}-NOSH" ${ENCRYPTIMAGE_TOOL_FOLDERNAME}

    cd ${ENCRYPTIMAGE_TOOL_FOLDERNAME}
    ./cbnEncryptImage -i "${VERSION_PROJECT}-NOSH-PA" -o "${VERSION_PROJECT}-NOSH.p7"
    echo "Ready to enter Sync_NOSH_Image_To_Release_Folder"
